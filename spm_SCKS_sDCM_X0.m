...
% ------------------------------------------------------------------------
inE    = SCKS.pU.v;            % observations
if size(inE,1)>size(inE,2)     % check the dimensions
    inE = inE';
end
% intrepolate if dt < 1:
% y    = interp1([1:size(y,2)],y',[1:1/nD:size(y,2)],'linear')';
inE    = spm_interp(inE',1/dt)';
if size(inE,1)>size(inE,2)       % check dimensions again
    inE = inE';
end
T    = size(inE,2);            % number of time points 

% =========================================================================
% interpolate input according to integration step
%--------------------------------------------------------------------------
...
% =========================================================================
% interpolate confounds according to integration step
%--------------------------------------------------------------------------
cf     = SCKS.cf.cf;            % observations
if size(y,1)>size(y,2)     % check the dimensions
    cf = cf';
end
for ntmp=1:size(cf,1)
  cftmp(ntmp,:)    = spm_interp(cf(ntmp,:)',1/dt)';
end
if size(cftmp,1)>size(cftmp,2)       % check dimensions again
    cftmp = cftmp';
end
cf=cftmp;
T    = size(cf,2);            % number of time points 
...
sWCF=sparse(real(spm_sqrtm(SCKS.cf.pP)));
Swcf=sparse(real(spm_sqrtm(SCKS.cf.wP)));
...
% nu      = size(sV,1);             
nu      =size (sWCF,1);           % number of confounds parameter
...
for run = 1:RUN
    ...
    for t = 2:T,
        ...
        XiR(:)        = Xi(xind1,:);
        f             = M(1).f(XiR,inE(t),pE);
 
        % integration by local-linearization scheme:
        %------------------------------------------------------------------
        dfdx           = spm_diff_all(M(1).f,XiR,inE(t),pE,1);
        ...
        S             = spm_qr([X0 blkdiag(sQ,sWCF,sW{t-1})]');
        ...
        yPred(:)      = M(1).g(XiR,inE(t),pE,XiCF,cf(:,t));
        y1            = sum(yPred,2)/nPts;
    ...
    if smoother
        ...
        for t = T-1:-1:1
            ...
            S             = spm_qr([Xf{t} blkdiag(sQ*0.9,sWCF,sW{T-1})]');
            ...
            resid         = xx(:,t+1) - x1f(:,t);
            xx(:,t)       = xx(:,t) + K*resid;
            S             = spm_qr([X01 - K*Xf{t}, K*blkdiag(sQ,sWCF,sW{T-1}), K*Sc{t+1}]');
...
%             end
            ...
            wcfTmp=mean(XXb(uind,:),2);
            yy       = reshape(M(1).g(reshape(XXb(xind1,:),nx/N,N*T),XXb(uind,:),repmat(pE(:,1),1,T),wcfTmp,cf),no,T);
            res      = y - yy;
